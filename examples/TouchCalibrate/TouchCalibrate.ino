/*
 * TouchCalibrate - guided resistive-touch setup tool for the CYD 2.4".
 *
...
chipguy_cyd2432S024_display display;
chipguy_cyd2432S024_touch  touch;

static const int SCRW = 240;
static const int SCRH = 320;

// Direct-blit colors (RGB565; panel is RGB order on this board).
...
static float runCalibration(int cols, int rows,
    ...
    float ax, bx, ay, by;
    linfit(gy, rawXs, n, ax, bx);   // rawX = ax + bx*Ty (long/320 axis)
    linfit(gx, rawYs, n, ay, by);   // rawY = ay + by*Tx (short/240 axis)

    xMin = clampRaw(ax);                    // rawX at Ty=0
    xMax = clampRaw(ax + bx * (SCRH - 1));  // rawX at Ty=319
    yMax = clampRaw(ay);                    // rawY at Tx=0  (left)
    yMin = clampRaw(ay + by * (SCRW - 1));  // rawY at Tx=239 (right)
    ...
    for (int i = 0; i < n; i++) {
        float predTy = (xMax != xMin) ? (rawXs[i] - xMin) * (SCRH - 1.0f) / (xMax - xMin) : 0;
        float preSx  = (yMax != yMin) ? (rawYs[i] - yMin) * (SCRW - 1.0f) / (yMax - yMin) : 0;
        float predTx = (SCRW - 1) - preSx;
        float ex = predTx - gx[i], ey = predTy - gy[i];
